...
function wysiwyg_textarea($name, $value, $type = 'apadana', $class = null, $extra = null, $cols = 50, $rows = 10)
{
	//These Lines Needed in all types , So Why dont set them First????
	...
	$return = '<textarea name="'.$name.'" id = "'.$id.'" rows="'.$rows.'" cols="'.$cols.'" class="'.$class.'" '.$extra.' >'.$value.'</textarea>';

	$return .= "<script type=\"text/javascript\">//<![CDATA[\n window.CKEDITOR_BASEPATH='". (url.'engine/editor/ckeditor/') ."';";

	$config['smiley_path'] = url.'engine/images/smiles/';
	$config['uiColor'] = $options['editor-color'];

	$config['smiley_images'] = array(
	...
	$config['smiley_descriptions'] = array(
	...
	switch($type)
	{
		...
		{
			global $options;
			$config['filebrowserBrowseUrl'] = url.'?admin='.$options['admin'].'&section=media&noTemplate=true&editor=true';
		}		
		break;
	...
	$return  .= "CKEDITOR.replace('".$id."', ".json_encode($config)."); \n//]]></script>";
	return $return;
}
